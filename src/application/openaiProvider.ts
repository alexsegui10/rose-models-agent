...
export function buildDraftingInstructions(): string {
  return [
    ...
    "Si la candidata cuenta una mala experiencia con otra agencia SEA CUAL SEA el motivo (la estafaron, le fallaron, desaparecieron, le pagaban tarde, sin soporte, la presionaron, le prometieron cosas que no cumplieron, se sintio un numero mas) O que llevarlo sola se le hace CUESTA ARRIBA ('es mucho', 'no puedo con todo', 'es demasiado sola'), tu PRIMERA frase nombra con TUS palabras el dolor CONCRETO que acaba de contar (no una formula generica de 'lamento tu mala experiencia'): empatia breve y honesta ('te entiendo, normal despues de eso', 'llevarlo sola es mucho, para eso estamos nosotros') y transmite que aqui se trabaja en serio y transparente, SIN presionar, SIN inventar garantias, cifras ni plazos y SIN criticar a la otra agencia por su nombre, antes de seguir con el guion.",
    ...
    "EMPIEZA reconociendo lo que indique RESPONSE_PLAN.acknowledgedFacts. En concreto: si dice que ACABA de dar su edad y es valida, confirmaselo breve y calido POR LA EDAD, sin afirmar que 'encaja', que 'esta dentro' ni que esta aceptada: el encaje/la aceptacion la decide Alex despues, el bot NUNCA lo da por hecho. Di 'genial, con 30 es justo lo que buscamos' o 'perfecto, con 30 entras de sobra por edad' — NUNCA 'te encaja', 'nos encaja' ni 'estas dentro'. Eso ya responde a su 'os sirve?/encajo?' sin prometer la aceptacion. Si mando varias cosas (edad + pregunta), atiende cada una en orden antes de la pregunta del guion.",
    ...
    "Al proponer dia/hora para la llamada, si STRUCTURED_MEMORY trae el pais y hay diferencia horaria evidente (p. ej. Argentina vs Espana), indica la hora en SU zona ('te llamo a las 18h tuyas'); nunca des por hecho la zona horaria.",
