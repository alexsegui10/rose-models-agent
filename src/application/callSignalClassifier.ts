...
const FOLLOWUP_SHARE_COMPLAINT_EXTRA =
  /\b(?:me\s+(?:quedo|queda|llevo|deja\w*)|sigue(?:\s+siendo)?)\s+(?:con\s+)?(?:muy\s+|medio\s+|re\s+)?(?:cort\w*|floj\w*|escas\w*|justit\w*|abajo|baj[oa])\b|\bun(?:\s+puntito|\s+punto|\s+termino)?\s+medio\b|\bpart\w*\s+la\s+diferencia\b|(?<!\bno\s)(?<!\bno\s\w{1,3}\s)(?:\bte\s+|\bque\s+|\byo\s+)?\bquiero\s+(?:un\s+poco\s+)?mas\b(?!\s*(?:que\s+nada|info\w*|detall\w*|foto\w*|video\w*|tiempo|contenido|contexto|saber|datos?|explicac\w*|vueltas?|rollos?|charla|historias?))|\b(?:subi\w*|dame|darme|ponme|dejame\w*|mejora\w*(?:me|lo)?|necesito)\s+(?:me\s+|nos\s+|hasta\s+|a\s+|en\s+|el\s+|un\s+|aunque\s+sea\s+)*(?:4\d|5\d|6\d)\b(?!\s*(?:fotos?|reels?|dias?|videos?|minutos?|horas?|semanas?|mes(?:es)?|anos?|seguidor))|\b(?:4\d|5\d|6\d)\s+(?:aunque\s+sea|y\s+(?:cerramos|arreglamos|listo)|para\s+mi\b(?![^.?!]{0,15}\b(?:bien|perfecto|genial|joya|barbaro|listo|ok|me\s+sirve|me\s+vale)\b))|\bcon\s+el\s+(?:%|porcentaje)\b[^.?!]{0,25}\b(?:floj\w*|cort\w*|poc\w*|convenc\w*)\b|\b(?:porfa|porfis|porfavor|por\s+favor)\s+algo\s+mas\b(?!\s+de\s+(?:tiempo|info\w*|detall\w*|foto\w*|video\w*|contenido|contexto|datos?|explicac\w*))|\baunque\s+sea\s+un\s+(?:poquito|toque|cachito|pelin)\s+mas\b(?!\s+de\s+(?:tiempo|info\w*|detall\w*|foto\w*|video\w*|contenido|contexto|datos?|explicac\w*))/;
// + "porfa algo más" / "aunque sea un poquito/toque más" (barrido Fase 3, Abril): en moneyContext tras la
// defensa es una continuación de regateo inequívoca; caía a ingresos y la escalera no seguía.
